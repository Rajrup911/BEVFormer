...
def custom_atan2(y, x):
    # torch atan2 function optimised for onnx export
    ...
    ans += torch.where(y_positive & x_negative, pi, torch.zeros_like(ans))
    ans -= torch.where(y_negative & x_negative, pi, torch.zeros_like(ans))
    ans = torch.where(y_positive & x_zero, half_pi, ans)
...
def custom_rotate(img, angle, center = None):
    """
    Rotate the image by angle. (Modified from Torchvision Rotate for Onnx Export)
...
class PerceptionTransformer(BaseModule):
        ...
        if prev_bev is not None:
            ...
            if self.rotate_prev_bev:
                for i in range(bs):
                    ...
                    tmp_prev_bev, grid = custom_rotate(tmp_prev_bev, torch.tensor(rotation_angle, dtype=torch.float32),
    ...
    def get_bev_features_export(
        ...
        ego_angle = kwargs['img_metas'][0]['can_bus'][-2].unsqueeze(0).float() / torch.pi * 180
        
        grid_length_y = grid_length[0]
        grid_length_x = grid_length[1]
        translation_length = torch.sqrt(delta_x ** 2 + delta_y ** 2)
        translation_angle = custom_atan2(delta_y, delta_x) / torch.pi * 180
        ...
        if prev_bev is not None:
            ...
            if self.rotate_prev_bev:
                for i in range(bs):
                    ...
                    tmp_prev_bev = prev_bev.squeeze(1).reshape(bev_h, bev_w, -1).permute(2, 0, 1)
        ...
        # add can bus signals
        can_bus = kwargs['img_metas'][0]['can_bus'].unsqueeze(0).float() ##([each['can_bus'].cpu().numpy() for each in kwargs['img_metas']])   [:, :]
        can_bus = self.can_bus_mlp(can_bus)[None, :, :]
    ...
    def forward(self,
        ...
        Returns:
            tuple[Tensor]: results of decoder containing the follwing tensor.
